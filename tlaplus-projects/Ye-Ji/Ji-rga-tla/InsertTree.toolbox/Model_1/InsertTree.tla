...
Readtree2list(T,root,tomb,readchar) == 
                    IF \E x \in T : x.father = root /\ ~ x.ch \in tomb /\ x.time = max (T,root,0,readchar) /\ ~ x.ch \in readchar
                         THEN   LET i == CHOOSE x \in T : x.father = root /\ ~ x.ch \in tomb /\ x.time = max (T,root,0,readchar) /\ ~ x.ch \in readchar
                            IN <<i.ch>> \o Readtree2list(T, i.ch, tomb, readchar \cup{i.ch} ) 
                    ELSE IF \E x \in T : x.father = root /\ x.ch \in tomb /\ x.time = max (T,root,0,readchar) /\ ~ x.ch \in readchar
                         THEN   LET i == CHOOSE x \in T : x.father = root /\  x.ch \in tomb /\ x.time = max (T,root,0,readchar) /\ ~ x.ch \in readchar
...
\* Last modified Sun Dec 16 15:23:50 CST 2018 by xhdn
