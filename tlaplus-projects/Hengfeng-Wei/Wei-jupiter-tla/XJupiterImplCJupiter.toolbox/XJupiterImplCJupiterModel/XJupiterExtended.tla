...
VARIABLES
    ...
    *)
    ...
    cincomingSerial, \* cincomingSerial[c]: the serival view received by client c \in Client from the Server
    sincomingSerial  \* (not used)

commCJVars == <<cincomingCJ, sincomingCJ>>
serialVars == <<serial, cincomingSerial, sincomingSerial>>
varsEx == <<cincomingCJ, serialVars, vars>>
-----------------------------------------------------------------------------
...
-----------------------------------------------------------------------------
(* 
Client c \in Client issues an operation.
*)
DoEx(c) == 
    /\ Do(c)
...
\* Last modified Wed Nov 07 11:37:42 CST 2018 by hengxin
