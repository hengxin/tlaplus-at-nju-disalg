...
This is a specification of the same system described by module
 \ensuremath{SendInt2}, except without the variable \ensuremath{z}. Each
 value to be sent is
 nondeterministically chosen by the \ensuremath{Send} action that sends it.
\end{cpar}%
\end{lcom}%
\@x{ {\EXTENDS} Integers}%
\@pvspace{8.0pt}%
...
\@x{ {\VARIABLE} x}%
\@pvspace{8.0pt}%
\@x{ TypeOK \.{\defeq} x \.{\in} Int \.{\cup} \{ NotInt \}}%
\@pvspace{8.0pt}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 \ensuremath{Oct} 19 02:28:14 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Tue} \ensuremath{Sep} 06
 02:11:16 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\end{lcom}%
\end{document}
