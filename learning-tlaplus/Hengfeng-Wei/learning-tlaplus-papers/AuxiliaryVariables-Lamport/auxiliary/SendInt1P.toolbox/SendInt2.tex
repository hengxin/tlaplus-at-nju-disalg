...
The example in Section 4.1 of the paper ``Auxiliary Variables in TLA+''
 comprises this module and modules \ensuremath{SendInt1} and
 \ensuremath{SendInt1P}.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
The example explains the basic idea behind prophecy variables with a
 ...
 arbitrary integers to a receiver, where sending an integer \ensuremath{v}
 means
 setting the variable \ensuremath{x} to \ensuremath{v}, and receiving the
 integer means setting \ensuremath{x
 } to the non-integer value \ensuremath{NotInt}. This spec also contains a
 variable \ensuremath{z
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 This defines \ensuremath{NotInt} to be some particular constant value that is
 not an
 integer. The semantics of TLA+ do not determine what that particular
 value is, just that it isn\mbox{'}t an integer. It is the same value for
 every possible behavior satisfying the spec. By default, when creating
 a model, \ensuremath{TLC} substitutes a model value of the same name for
 \ensuremath{NotInt}.
...
In general, a spec should define a formula that asserts type
 ...
 spec, and you can catch simple ``type'' errors easily by having
 \ensuremath{TLC} check
...
\end{cpar}%
\end{lcom}%
\@x{ TypeOK \.{\defeq} \.{\land} x \.{\in} Int \.{\cup} \{ NotInt \}}%
\@x{\@s{56.14} \.{\land} z\@s{0.52} \.{\in} Int \.{\cup} \{ NotInt \}}%
\@pvspace{8.0pt}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 \ensuremath{Oct} 19 02:48:18 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Tue} \ensuremath{Sep} 06
 02:16:47 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\end{lcom}%
\end{document}
