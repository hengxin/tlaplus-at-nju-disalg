...
This module is part of the example from the paper ``Auxiliary Variables
 in TLA+'' that also includes module \ensuremath{SendInt1} and
 \ensuremath{SendInt2}. It adds a
 one-prediction prophecy variable \ensuremath{p} to specification
 \ensuremath{Spec} of \ensuremath{SendInt1} to
 obtain specification \ensuremath{SpecP}, and it defines a refinement mapping
 under
 which \ensuremath{SpecP} implements specification \ensuremath{Spec} of
 module \ensuremath{SendInt2}.
\end{cpar}%
\end{lcom}%
\@x{ {\EXTENDS} SendInt1}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
Pi is the set of possible values of (predictions made by) \ensuremath{p}.
\end{cpar}%
\end{lcom}%
\@x{ Pi \.{\defeq} Int}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The operator \ensuremath{PredSend} is used in the definition of
 \ensuremath{SendP} below. We
 define it before the declaration of the variable \ensuremath{p} to allow us
 more
 ...
 \ensuremath{{\EE} p} : \ensuremath{SpecP} (the specification obtained by
 hiding \ensuremath{p} in \ensuremath{SpecP}) is
 equivalent to \ensuremath{Spec}. To check this theorem with
 \ensuremath{TLC}, temporarily end
 the module by adding few ``\ensuremath{\.{=}}'' characters after the theorem
 and create a
 model having \ensuremath{Spec} as the specification.
\end{cpar}%
\end{lcom}%
\@x{ PredSend ( i ) \.{\defeq} x \.{'} \.{=} i}%
\@pvspace{8.0pt}%
 \@x{ {\THEOREM} Spec \.{\implies} {\Box} [ Send \.{\implies} \E\, i \.{\in}
 Pi \.{:} PredSend ( i ) ]_{ x}}%
\@x{}\midbar\@xx{}%
\@x{ {\VARIABLE} p}%
\@pvspace{8.0pt}%
\@x{ varsP \.{\defeq} {\langle} x ,\, p {\rangle}}%
\@pvspace{8.0pt}%
\@x{ InitP\@s{2.14} \.{\defeq}\@s{4.1} Init \.{\land} ( p \.{\in} Pi )}%
\@pvspace{8.0pt}%
\@x{ TypeOKP \.{\defeq} TypeOK\@s{4.1} \.{\land} ( p \.{\in} Pi )}%
\@pvspace{8.0pt}%
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The theorem below asserts that \ensuremath{SpecP} implements
 \ensuremath{SendInt2} under the
 refinement mapping defined by the \ensuremath{{\INSTANCE}} statement.
\end{cpar}%
\end{lcom}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 00:37:21 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Tue} \ensuremath{Sep} 06
 02:21:32 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\end{lcom}%
\end{document}
