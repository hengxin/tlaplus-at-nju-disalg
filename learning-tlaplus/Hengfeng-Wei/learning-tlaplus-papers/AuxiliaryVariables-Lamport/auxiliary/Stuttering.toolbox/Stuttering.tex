...
This module is explained in Section 5 of the paper ``Auxiliary Variables
 in TLA+''. It defines operators used to add a stuttering variable
 \ensuremath{s} to
 a specification \ensuremath{Spec} to form a specification
 \ensuremath{SpecS}. It is mean to be
 instantiated with \ensuremath{s} replaced by the stuttering variable to be
 added and
 \ensuremath{vars} replaced by the tuple of all variables in the original
 specification.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 If \ensuremath{Init} is the initial predicate of \ensuremath{Spec} , then the
 initial predicate
 of \ensuremath{SpecS} is \ensuremath{Init \.{\land}} (\ensuremath{s \.{=}
 top}) , where top is defined below.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The next-state action of \ensuremath{SpecS} is obtained by replacing each
 subaction
 \ensuremath{A} of a disjunctive representation of the next-state action Next
 of
 \ensuremath{Spec} with an action \ensuremath{AS} written in terms of
 operators defined below.
 (Disjunctive representations are described in Section 3.2 of ``Auxiliary
 Variables in TLA+''.) Action \ensuremath{AS} executes \ensuremath{A} and
 stuttering steps
 added either before or after an \ensuremath{A} step. The basic idea is that
 \ensuremath{s
...
\ensuremath{id}: A value that identifies the action \ensuremath{A} .
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{T}{T}{0}{2.5}{%
\ensuremath{ctxt}:}%
A value identifying the context under which \ensuremath{A} is executed.
 For example, if \ensuremath{A} appears in a formula \ensuremath{\E\, i,\, j
 \.{\in} S \.{:} A} ,
 this would equal the value of the pair \ensuremath{{\langle}i,\, j{\rangle}}
 for
 which \ensuremath{A} is being executed.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{1}{T}{T}{0}{2.5}{%
\ensuremath{val}:}%
A value that is decremented by each stuttering step, until
...
\end{cpar}%
\begin{cpar}{0}{T}{F}{5.0}{0}{}%
The subaction \ensuremath{A} of Next.
\end{cpar}%
\vshade{5.0}%
...
\end{cpar}%
\begin{cpar}{0}{T}{F}{5.0}{0}{}%
A string identifying action \ensuremath{A} .
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{1}{F}{F}{0}{0}{}%
\ensuremath{Sigma
...
A set of values ordered by some ``less-than'' relation. This is
 the set of possible values of \ensuremath{s.val} when executing stuttering
 steps before or after subaction \ensuremath{A} .
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{1}{F}{F}{0}{0}{}%
\ensuremath{bot
...
\end{cpar}%
\begin{cpar}{0}{T}{F}{5.0}{0}{}%
The minimum element of \ensuremath{Sigma} under its less-than relation.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{1}{F}{F}{0}{0}{}%
\ensuremath{initVal
...
\end{cpar}%
\begin{cpar}{0}{T}{F}{5.0}{0}{}%
The initial value to which \ensuremath{s.val} is set for executing stuttering
 steps before or after \ensuremath{A} .
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{1}{F}{F}{0}{0}{}%
\ensuremath{decr
...
\end{cpar}%
\begin{cpar}{0}{T}{F}{5.0}{0}{}%
An operator such that each stuttering step changes \ensuremath{s.val} to
 \ensuremath{decr(s.val)}.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{1}{F}{F}{0}{0}{}%
\ensuremath{context
...
\end{cpar}%
\begin{cpar}{0}{T}{F}{5.0}{0}{}%
The context in which \ensuremath{A} appears. It is the expression that is
 evaluated to determine the value to which \ensuremath{s.ctxt} is set.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{1}{F}{F}{0}{0}{}%
\ensuremath{enabled
...
\end{cpar}%
\begin{cpar}{0}{T}{F}{5.0}{0}{}%
 A formula equivalent to \ensuremath{{\ENABLED} A} . You can always take it to
 be
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
\ensuremath{{\ENABLED} A} , but you can usually find an expression that equals
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{{\ENABLED} A} in every reachable state of \ensuremath{Spec} but
 is easier for
 \ensuremath{TLC} to compute.
\end{cpar}%
\end{lcom}%
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 Equals \ensuremath{AS} when no stuttering steps are being added before or
 after
 \ensuremath{A} .
\end{cpar}%
\end{lcom}%
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The \ensuremath{PostStutter} and \ensuremath{PreStutter} operators define
 actions that perform
 stuttering steps after xecuting \ensuremath{A} and before executing
 \ensuremath{A} ,
 respectively. If \ensuremath{bot \.{=} 1}, \ensuremath{initVal \.{=} 42},
 and \ensuremath{decr(i) \.{=} i\.{-}1}, then the
...
\@x{\@s{8.2} {\IF} s \.{=} top \.{\THEN} \.{\land} A}%
 \@x{\@s{84.08} \.{\land} s \.{'} \.{=} [ id \.{\mapsto} actionId ,\, ctxt
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The operators \ensuremath{MayPostStutter} and \ensuremath{MayPreStutter} are
 like \ensuremath{PostStutter} and
 \ensuremath{PreStutter}, except they add one fewer stuttering step. For
 example, if
 \ensuremath{bot \.{=} 1}, \ensuremath{initVal \.{=} 42}, and
 \ensuremath{decr(i) \.{=} i\.{-}1}, then they add 42 steps. If
 \ensuremath{initVal \.{=} bot}, then they simply execute A without any added
 stuttering
 steps.
\end{cpar}%
\end{lcom}%
...
\@x{\@s{67.02} \.{\THEN} A \.{\land} ( s \.{'} \.{=} s )}%
 \@x{\@s{67.02} \.{\ELSE} \.{\land} s \.{'} \.{=} [ id \.{\mapsto} actionId
 ,\, ctxt \.{\mapsto} context ,\, val \.{\mapsto} initVal ]}%
\@x{\@s{98.33} \.{\land} {\UNCHANGED} vars}%
\@x{\@s{16.4} \.{\ELSE} \.{\land} s . id \.{=} actionId}%
 \@x{\@s{47.71} \.{\land} {\IF} s . val \.{=} bot \.{\THEN} \.{\land} s . ctxt
 \.{=} context}%
\@x{\@s{150.07} \.{\land} A}%
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The following operator is true iff repeatedly apply \ensuremath{decr} to any
 element
 \ensuremath{sig} of \ensuremath{Sigma} produces a sequence \ensuremath{sig},
 \ensuremath{decr(sig)}, \ensuremath{decr(decr(sig))}, \.{\dots}
 that eventually arrives at \ensuremath{bot}. This condition should be
 satisfied by
 the values of \ensuremath{Sigma}, \ensuremath{bot}, and \ensuremath{decr}
 used with the operators defined
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{TLC} can evaluate \ensuremath{StutterConstantCondition} only in a
 model that replaces
 \ensuremath{Nat} by the set \ensuremath{0\.{\dotdot}n} for some integer
 \ensuremath{n}. The following operator is
 equivalent to \ensuremath{StutterConstantCondition} when \ensuremath{Sigma}
 is a finite set, but
 doesn\mbox{'}t require modifying any definitions.
\end{cpar}%
\end{lcom}%
 \@x{ AltStutterConstantCondition ( Sigma ,\, bot ,\, decr ( \_ ) )
 \.{\defeq}}%
 \@x{\@s{12.29} \.{\LET} InverseDecr ( S ) \.{\defeq} \{ sig \.{\in} Sigma
 \.{\,\backslash\,} S \.{:} decr ( sig ) \.{\in} S \}}%
\@x{\@s{36.79} ReachBot [ S \.{\in} {\SUBSET} Sigma ] \.{\defeq}}%
\@x{\@s{49.09} {\IF} InverseDecr ( S ) \.{=} \{ \} \.{\THEN} S}%
\@x{\@s{152.30} \.{\ELSE} ReachBot [ S \.{\cup} InverseDecr ( S ) ]}%
\@x{\@s{12.29} \.{\IN}\@s{4.09} ReachBot [ \{ bot \} ] \.{=} Sigma}%
\@x{}\bottombar\@xx{}%
\setboolean{shading}{false}
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Last modified Sat \ensuremath{Dec} 31
 17:47:02 \ensuremath{PST} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Tue} \ensuremath{Dec} 08
 11:51:34 \ensuremath{PST} 2015 by \ensuremath{lamport
}%
\end{cpar}%
\end{lcom}%
\end{document}
