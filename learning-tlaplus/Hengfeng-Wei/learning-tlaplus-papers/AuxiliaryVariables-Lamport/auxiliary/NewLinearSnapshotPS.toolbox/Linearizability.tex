...
A data object, also called a state machine, executes commands from user
 processses. It is described by an initial state \ensuremath{InitObj} of the
 object
 ...
 \ensuremath{Commands(i)} and \ensuremath{Outputs(i)} for
 each process \ensuremath{i}, and a set \ensuremath{ObjValues} of all
 possible states of the
 object, and an initial process \ensuremath{i} output value
 \ensuremath{InitOutput(i)} whose use
 is explained below. Here are the declarations of these constants and
 constant operators and the property they are assumed to satisfy. The
 requirement that \ensuremath{Outputs(i)} and \ensuremath{Commands(i)} are
 disjoint sets is not
 essential, but makes our specification of linearizability simpler.
\end{cpar}%
\end{lcom}%
...
The definitions of the intial predicate and next-state action should be
 easy to understand.
\end{cpar}%
\end{lcom}%
...
\@pvspace{8.0pt}%
\@x{ TypeOK \.{\defeq} \.{\land} object \.{\in} ObjValues}%
 \@x{\@s{56.14} \.{\land} \A\, i \.{\in} Procs \.{:} \.{\land} interface [ i ]
 \.{\in} Outputs ( i ) \.{\cup} Commands ( i )}%
 \@x{\@s{124.82} \.{\land} istate [ i ] \.{\in} Outputs ( i ) \.{\cup}
 Commands ( i )}%
\@pvspace{8.0pt}%
\@x{ Init \.{\defeq} \.{\land} object\@s{6.00} \.{=} InitObj}%
 \@x{\@s{35.70} \.{\land} interface \.{=} [ i \.{\in} Procs \.{\mapsto}
 InitOutput ( i ) ]}%
 \@x{\@s{35.70} \.{\land} istate \.{=} [ i \.{\in} Procs \.{\mapsto}
 InitOutput ( i ) ]}%
\@pvspace{8.0pt}%
...
\end{cpar}%
\end{lcom}%
 \@x{ Fairness \.{\defeq} \A\, i \.{\in} Procs \.{:} {\WF}_{ vars} ( DoOp ( i
 ) ) \.{\land} {\WF}_{ vars} ( EndOp ( i ) )}%
\@x{ Spec \.{\defeq} SafeSpec \.{\land} Fairness}%
\@x{}\bottombar\@xx{}%
\setboolean{shading}{false}
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 \ensuremath{Oct} 19 09:24:37 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
