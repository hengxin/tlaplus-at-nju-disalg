...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 This module and modules \ensuremath{SendSeqUndo} and
 \ensuremath{SendSeqUndoP} form the \ensuremath{SendSeq
} example from Section 4.4 of the paper ``Auxiliary Variables in TLA+''.
 It is a variant of the \ensuremath{SendSet} example in modules
 \ensuremath{SendSet}, \ensuremath{SendSetUndo},
 and \ensuremath{SendSetUndoP}. The difference between the two examples is
 that the
 value of variable \ensuremath{y} in \ensuremath{SendSet} is a set of data
 values that may be sent
 in any order. In the current \ensuremath{SendSeq} example, the value of
 \ensuremath{y} is a
 sequence of data values that are to be sent in order. The \ensuremath{Undo(S)
 } action of \ensuremath{SendSetUndo} that removes the set \ensuremath{S} of
 data values from the
 set \ensuremath{y} is replaced in \ensuremath{SendSeqUndo} by an
 \ensuremath{Undo(i)} action that removes
 element number \ensuremath{i} from the sequence \ensuremath{y}.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
If you understand module \ensuremath{SendSet}, you should have no problem
...
\@x{\@s{56.14} \.{\land} y\@s{0.10} \.{\in} Seq ( Data )}%
\@pvspace{8.0pt}%
 \@x{ Init \.{\defeq} ( x \.{=} NonData ) \.{\land} ( y \.{=} {\langle}
 {\rangle} )}%
\@pvspace{8.0pt}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 \ensuremath{Oct} 19 05:15:04 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Thu} \ensuremath{Sep} 15
 02:12:27 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\end{lcom}%
\end{document}
