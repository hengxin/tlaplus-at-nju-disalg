...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 This module adds a prophecy variable \ensuremath{p} to specification
 \ensuremath{SpecU} of module
 \ensuremath{SendSeqUndo} to obtain the specification \ensuremath{SpecUP}. It
 then asserts that
 \ensuremath{SpecUP} implements specification \ensuremath{Spec} of module
 \ensuremath{SendSeq} under a suitable
 refinement mapping, which implies that \ensuremath{SpecU} implements
 \ensuremath{{\EE} y} : \ensuremath{Spec}.
\end{cpar}%
\end{lcom}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 individual
 predictions and the domain \ensuremath{Dom} of \ensuremath{p}, where
 \ensuremath{p[d]} makes a prediction
 associated with \ensuremath{d}. In this case, \ensuremath{d} is the domain
 of \ensuremath{y} (which equals
 \ensuremath{1\.{\dotdot}Len(y)}), and \ensuremath{p[d]} predicts whether
 element number \ensuremath{d} of \ensuremath{y} is either
 sent or undone (removed from \ensuremath{y} by an \ensuremath{Undo} step).
\end{cpar}%
\end{lcom}%
\@x{ Pi \.{\defeq} \{\@w{send} ,\,\@w{undo} \}}%
\@x{ Dom \.{\defeq} {\DOMAIN} y}%
\@pvspace{8.0pt}%
 \@x{ {\INSTANCE} Prophecy {\WITH} Pi \.{\leftarrow} \{\@w{send} ,\,\@w{undo}
 \} ,\, DomPrime \.{\leftarrow} Dom \.{'}}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 Adding the prophecy variable requires replacing each subaction \ensuremath{A}
 of a
 disjunctive representation with an action \ensuremath{Ap}. We use the
 disjunctive
 representation with subactions \ensuremath{Choose}, \ensuremath{Send},
 \ensuremath{Rcv}, and \ensuremath{Undo(i)}. Each
 action \ensuremath{Ap} is defined by defining:
\end{cpar}%
\vshade{5.0}%
...
\ensuremath{\.{-}}}%
 ...
 \ensuremath{p\.{'}}.
 For the prophecy variable we are defining, \ensuremath{DomInjA} specifies the
 obvious correspondence between the elements of the sequences \ensuremath{y}
 and
 \ensuremath{y\.{'}}.
\end{cpar}%
\vshade{5.0}%
...
These definitions for each subaction \ensuremath{A} follow. For example,
 \ensuremath{PredDomChoose} is \ensuremath{PredDomA} for \ensuremath{A} the
 \ensuremath{Choose} action.
\end{cpar}%
\end{lcom}%
\@x{ PredDomChoose \.{\defeq} \{ \}}%
\@x{ DomInjChoose\@s{7.14} \.{\defeq} [ i \.{\in} Dom \.{\mapsto} i ]}%
...
The following theorem asserts the action requirements described in
 ...
 to ensure that \ensuremath{{\EE} p} : \ensuremath{SpecUP} is equivalent to
 \ensuremath{SpecU}.
\end{cpar}%
\end{lcom}%
 \@x{ Condition\@s{4.1} \.{\defeq} \.{\land} ProphCondition ( Choose ,\,
 DomInjChoose ,\, PredDomChoose ,\,}%
\@x{\@s{151.72} PredChoose )}%
 \@x{\@s{66.94} \.{\land} ProphCondition ( Send ,\, DomInjSend ,\, PredDomSend
 ,\, PredSend )}%
 \@x{\@s{66.94} \.{\land} ProphCondition ( Rcv ,\, DomInjRcv ,\, PredDomRcv
 ,\, PredRcv )}%
\@x{\@s{66.94} \.{\land} \A\, i \.{\in} Dom \.{:}}%
 \@x{\@s{85.27} ProphCondition ( Undo ( i ) ,\, DomInjUndo ( i ) ,\,
 PredDomUndo ( i ) ,\,}%
\@x{\@s{158.94} {\LAMBDA} p \.{:} PredUndo ( p ,\, i ) )}%
\@pvspace{8.0pt}%
\@x{ {\THEOREM} SpecU \.{\implies} {\Box} [ Condition ]_{ vars}}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
Temporarily end the module here to use \ensuremath{TLC} to check the theorem.
\end{cpar}%
\end{lcom}%
\@x{}\midbar\@xx{}%
\@x{ {\VARIABLE} p}%
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The actions \ensuremath{Ap} are defined using the \ensuremath{ProphAction}
 operator defined in
 the \ensuremath{Prophecy} module.
\end{cpar}%
\end{lcom}%
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The theorem below asserts that \ensuremath{SpecUP} implements specification
 \ensuremath{Spec} of
 module \ensuremath{SendSeq} under the refinement mapping \ensuremath{y
 \.{\leftarrow} yBar}, where \ensuremath{yBar} is
 defined here to be the subsequence of \ensuremath{y} consisting of those
 elements
 for which \ensuremath{p} predicts \ensuremath{\@w{send}}.
\end{cpar}%
\end{lcom}%
\@x{ yBar \.{\defeq}\@s{4.1} \.{\LET} {\RECURSIVE} R ( \_ ,\, \_ )}%
\@x{\@s{69.78} R ( yseq ,\, pseq ) \.{\defeq}}%
\@x{\@s{85.45} {\IF} yseq \.{=} {\langle} {\rangle}}%
\@x{\@s{93.65} \.{\THEN} yseq}%
\@x{\@s{93.65} \.{\ELSE} {\IF} Head ( pseq ) \.{=}\@w{send}}%
 \@x{\@s{133.16} \.{\THEN} {\langle} Head ( yseq ) {\rangle} \.{\circ} R (
 Tail ( yseq ) ,\, Tail ( pseq ) )}%
\@x{\@s{133.16} \.{\ELSE}\@s{4.1} R ( Tail ( yseq ) ,\, Tail ( pseq ) )}%
\@x{\@s{45.28} \.{\IN}\@s{4.09} R ( y ,\, p )}%
\@pvspace{8.0pt}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 16:10:30 \ensuremath{PST} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Thu} \ensuremath{Sep} 15
 02:52:00 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\end{lcom}%
\end{document}
