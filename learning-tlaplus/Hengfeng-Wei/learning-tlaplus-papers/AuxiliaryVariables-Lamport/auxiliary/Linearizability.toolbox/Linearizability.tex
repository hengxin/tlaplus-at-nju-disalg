...
\setboolean{shading}{true}
 ...
 {\MODULE} Linearizability}\moduleRightDash\@xx{}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{Linearizability} was introduced by \ensuremath{Herlihy} and Wing
 in their 1987 \ensuremath{POPL
...
A data object, also called a state machine, executes commands from user
 processes. It is described by an initial state \ensuremath{InitObj} of the
 object
 and an operator \ensuremath{Apply}, where \ensuremath{Apply(i,\, cmd,\, st)}
 describes the output and
 new state of the object that results from process \ensuremath{i} executing
 command
 \ensuremath{cmd} when the object has state \ensuremath{st}. It is formally
 described by \ensuremath{Apply},
 \ensuremath{InitObj}, a set \ensuremath{Procs} of processes, sets
 \ensuremath{Commands(i)} and \ensuremath{Outputs(i)} of
 possible commands and possible outputs for each process \ensuremath{i}, a set
 \ensuremath{ObjValues} of all possible states of the object, and an initial
 process
 \ensuremath{i} output value \ensuremath{InitOutput(i)} whose use is
 explained below. Here are
 the declarations of these constants and constant operators and the
 property they are assumed to satisfy. The requirement that
 \ensuremath{Outputs(i)
 } and \ensuremath{Commands(i)} are disjoint sets is not essential, but makes
 our
 specification of linearizability simpler.
\end{cpar}%
\end{lcom}%
 \@x{\makebox[0pt][r]{\scriptsize 22\hspace{1em}} {\CONSTANTS}\@s{4.1} Procs
 ,\, Commands ( \_ ) ,\, Outputs ( \_ ) ,\, InitOutput ( \_ ) ,\,}%
 \@x{\makebox[0pt][r]{\scriptsize 23\hspace{1em}}\@s{58.85} ObjValues ,\,
 InitObj ,\, Apply ( \_ ,\, \_ ,\, \_ )}%
\@pvspace{8.0pt}%
 \@x{\makebox[0pt][r]{\scriptsize 25\hspace{1em}} {\ASSUME} LinearAssumps
 \.{\defeq}}%
 \@x{\makebox[0pt][r]{\scriptsize 26\hspace{1em}}\@s{46.44} \.{\land} InitObj
 \.{\in} ObjValues}%
 \@x{\makebox[0pt][r]{\scriptsize 27\hspace{1em}}\@s{46.44} \.{\land} \A\, i
 \.{\in} Procs \.{:} InitOutput ( i ) \.{\in} Outputs ( i )}%
 \@x{\makebox[0pt][r]{\scriptsize 28\hspace{1em}}\@s{46.44} \.{\land} \A\, i
 \.{\in} Procs \.{:} Outputs ( i ) \.{\cap} Commands ( i ) \.{=} \{ \}}%
 \@x{\makebox[0pt][r]{\scriptsize 29\hspace{1em}}\@s{46.44} \.{\land} \A\, i
 \.{\in} Procs ,\, obj \.{\in} ObjValues \.{:}}%
 \@x{\makebox[0pt][r]{\scriptsize 30\hspace{1em}}\@s{65.75} \A\, cmd \.{\in}
 Commands ( i ) \.{:}}%
 \@x{\makebox[0pt][r]{\scriptsize 31\hspace{1em}}\@s{77.07} \.{\land}\@s{3.71}
 Apply ( i ,\, cmd ,\, obj ) . output \.{\in} Outputs ( i )}%
 \@x{\makebox[0pt][r]{\scriptsize 32\hspace{1em}}\@s{77.07} \.{\land}\@s{3.71}
 Apply ( i ,\, cmd ,\, obj ) . newState \.{\in} ObjValues}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
A linearizable implementation of a data object is one in which each
 operation execution by a process \ensuremath{i} is performed by three steps:
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{T}{T}{2.5}{2.5}{%
-}%
An externally visible \ensuremath{BeginOp(i,\, cmd)} step that begins an
 operation with command \ensuremath{cmd}.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{1}{T}{T}{2.5}{2.5}{%
-}%
 An internal \ensuremath{DoOp(i)} step that performs the operation on the
 object
 and obtains the output.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{1}{T}{F}{2.5}{0}{}%
- An externally visible \ensuremath{EndOp(i)} step that displays the output.
\end{cpar}%
\vshade{5.0}%
...
\vshade{5.0}%
\begin{cpar}{1}{F}{F}{0}{0}{}%
\ensuremath{interface
}%
\end{cpar}%
\begin{cpar}{0}{T}{F}{7.5}{0}{}%
An externally visible variable whose value is a function with
 domain \ensuremath{Procs} that describes the issuing of commands and the
 return of values. The value of \ensuremath{interface[i]} is initially
 \ensuremath{InitOutput(i)}, it is set to \ensuremath{cmd} by the
 \ensuremath{BeginOp(i,\, cmd)} step,
 and is set to the command\mbox{'}s output by the \ensuremath{EndOp(i)} step.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{1}{F}{F}{0}{0}{}%
\ensuremath{istate
...
\end{cpar}%
\begin{cpar}{0}{T}{F}{7.5}{0}{}%
An internal variable whose value is a function with domain \ensuremath{Procs}.
 The element \ensuremath{istate[i]} is used to remember, when process
 \ensuremath{i} is
 executing a command, if it has executed the \ensuremath{DoOp(i)} step and,
 if so, what the output value is. It initially equals
 \ensuremath{InitOutput(i)}, is set to \ensuremath{cmd} by a
 \ensuremath{BeginOp(i,\, cmd)} step, and is
 set to the output value by a \ensuremath{DoOp(i)} step.
\end{cpar}%
\vshade{5.0}%
...
\end{cpar}%
\end{lcom}%
 \@x{\makebox[0pt][r]{\scriptsize 70\hspace{1em}} {\VARIABLES} object ,\,
 interface ,\, istate}%
 \@x{\makebox[0pt][r]{\scriptsize 71\hspace{1em}} vars \.{\defeq} {\langle}
 object ,\, interface ,\, istate {\rangle}}%
\@pvspace{8.0pt}%
 \@x{\makebox[0pt][r]{\scriptsize 73\hspace{1em}} TypeOK \.{\defeq} \.{\land}
 object \.{\in} ObjValues}%
 \@x{\makebox[0pt][r]{\scriptsize 74\hspace{1em}}\@s{56.14} \.{\land} \A\, i
 \.{\in} Procs \.{:} \.{\land} interface [ i ] \.{\in} Outputs ( i ) \.{\cup}
 Commands ( i )}%
 \@x{\makebox[0pt][r]{\scriptsize 75\hspace{1em}}\@s{124.82} \.{\land} istate
 [ i ] \.{\in} Outputs ( i ) \.{\cup} Commands ( i )}%
\@pvspace{8.0pt}%
 \@x{\makebox[0pt][r]{\scriptsize 77\hspace{1em}} Init \.{\defeq} \.{\land}
 object\@s{6.00} \.{=} InitObj}%
 \@x{\makebox[0pt][r]{\scriptsize 78\hspace{1em}}\@s{35.70} \.{\land}
 interface \.{=} [ i \.{\in} Procs \.{\mapsto} InitOutput ( i ) ]}%
 \@x{\makebox[0pt][r]{\scriptsize 79\hspace{1em}}\@s{35.70} \.{\land} istate
 \.{=} [ i \.{\in} Procs \.{\mapsto} InitOutput ( i ) ]}%
\@pvspace{8.0pt}%
 \@x{\makebox[0pt][r]{\scriptsize 81\hspace{1em}} BeginOp ( i ,\, cmd )
 \.{\defeq} \.{\land} interface [ i ] \.{\in} Outputs ( i )}%
 \@x{\makebox[0pt][r]{\scriptsize 82\hspace{1em}}\@s{93.61} \.{\land}
 interface \.{'} \.{=} [ interface {\EXCEPT} {\bang} [ i ] \.{=} cmd ]}%
 \@x{\makebox[0pt][r]{\scriptsize 83\hspace{1em}}\@s{93.61} \.{\land} istate
 \.{'}\@s{1.47} \.{=} [ istate {\EXCEPT} {\bang} [ i ] \.{=} cmd ]}%
 \@x{\makebox[0pt][r]{\scriptsize 84\hspace{1em}}\@s{93.61} \.{\land} object
 \.{'} \.{=} object}%
\@pvspace{8.0pt}%
 \@x{\makebox[0pt][r]{\scriptsize 86\hspace{1em}} DoOp ( i ) \.{\defeq}
 \.{\land} interface [ i ] \.{\in} Commands ( i )}%
 \@x{\makebox[0pt][r]{\scriptsize 87\hspace{1em}}\@s{56.82} \.{\land} istate [
 i ] \.{=} interface [ i ]}%
 \@x{\makebox[0pt][r]{\scriptsize 88\hspace{1em}}\@s{56.82} \.{\land} \.{\LET}
 result \.{\defeq} Apply ( i ,\, interface [ i ] ,\, object )}%
 \@x{\makebox[0pt][r]{\scriptsize 89\hspace{1em}}\@s{67.93} \.{\IN} \.{\land}
 object \.{'} \.{=} result . newState}%
 \@x{\makebox[0pt][r]{\scriptsize 90\hspace{1em}}\@s{88.33} \.{\land} istate
 \.{'}\@s{1.47} \.{=} [ istate {\EXCEPT} {\bang} [ i ] \.{=} result . output
 ]}%
 \@x{\makebox[0pt][r]{\scriptsize 91\hspace{1em}}\@s{56.82} \.{\land}
 interface \.{'} \.{=} interface}%
\@pvspace{8.0pt}%
 \@x{\makebox[0pt][r]{\scriptsize 93\hspace{1em}} EndOp ( i ) \.{\defeq}
 \.{\land} interface [ i ] \.{\in} Commands ( i )}%
 \@x{\makebox[0pt][r]{\scriptsize 94\hspace{1em}}\@s{61.67} \.{\land} istate [
 i ] \.{\in} Outputs ( i )}%
 \@x{\makebox[0pt][r]{\scriptsize 95\hspace{1em}}\@s{61.67} \.{\land}
 interface \.{'} \.{=} [ interface {\EXCEPT} {\bang} [ i ] \.{=} istate [ i ]
 ]}%
 \@x{\makebox[0pt][r]{\scriptsize 96\hspace{1em}}\@s{61.67} \.{\land}
 {\UNCHANGED} {\langle} object ,\, istate {\rangle}}%
\@pvspace{8.0pt}%
 \@x{\makebox[0pt][r]{\scriptsize 98\hspace{1em}} Next \.{\defeq} \E\, i
 \.{\in} Procs \.{:} \.{\lor} \E\, cmd \.{\in} Commands ( i ) \.{:} BeginOp (
 i ,\, cmd )}%
 \@x{\makebox[0pt][r]{\scriptsize 99\hspace{1em}}\@s{97.40} \.{\lor} DoOp ( i
 )}%
 \@x{\makebox[0pt][r]{\scriptsize 100\hspace{1em}}\@s{97.40} \.{\lor} EndOp (
 i )}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\end{cpar}%
\end{lcom}%
 \@x{\makebox[0pt][r]{\scriptsize 105\hspace{1em}} SafeSpec \.{\defeq} Init
 \.{\land} {\Box} [ Next ]_{ vars}}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
The liveness requirement is that an operation that has begun (by
 executing a \ensuremath{BeginOp(i,\, cmd)} step) must eventually finish (by
 executing
 an \ensuremath{EndOp(i)} step). This is expressed by the formulas
 \ensuremath{Fairness} defined
 below. The equivalent specification is obtained by defining
 \ensuremath{Fairness
} to equal this formula:
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{T}{F}{7.5}{0}{}%
 \ensuremath{\A\, i \.{\in} Procs} : \ensuremath{{\WF}\_vars(DoOp(i) \.{\lor}
 EndOp(i))
}%
\end{cpar}%
\end{lcom}%
 \@x{\makebox[0pt][r]{\scriptsize 116\hspace{1em}} Fairness \.{\defeq} \A\, i
 \.{\in} Procs \.{:} {\WF}_{ vars} ( DoOp ( i ) ) \.{\land} {\WF}_{ vars} (
 EndOp ( i ) )}%
 \@x{\makebox[0pt][r]{\scriptsize 117\hspace{1em}} Spec \.{\defeq} SafeSpec
 \.{\land} Fairness}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 01:28:13 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Tue} \ensuremath{Oct} 04
 02:01:02 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\end{lcom}%
\end{document}
