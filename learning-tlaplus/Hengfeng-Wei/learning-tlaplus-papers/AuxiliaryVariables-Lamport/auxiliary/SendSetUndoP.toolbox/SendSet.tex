...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 This module and modules \ensuremath{SendSetUndo} and
 \ensuremath{SendSetUndoP} are the
...
This module specifies a system in which a sender sends data values in
 an unspecified set \ensuremath{Data} to a receiver. A data value
 \ensuremath{d} is sent by
 setting the variable \ensuremath{x} to \ensuremath{d}, and it is received by
 setting \ensuremath{x} to a value
 \ensuremath{NotData} that is not an element of \ensuremath{Data}. The sender
 maintains a set \ensuremath{y
} of data values that is has chosen to send. The sender\mbox{'}s actions are
 \ensuremath{Choose} , which adds to \ensuremath{y} an arbitrary data value
 not already in \ensuremath{y}; and
 \ensuremath{Send} , which sends an arbitrarily chosen value in
 \ensuremath{y} and remove it from
 \ensuremath{y}. The definitions, leading up to the definition of the
 specification
 Spec, are straightforward.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 We consider \ensuremath{x} to be externally visible and \ensuremath{y} to be
 an internal
 variable. The sender\mbox{'}s \ensuremath{Send} action and the
 receiver\mbox{'}s \ensuremath{Rcv} action,
 which change \ensuremath{x}, are therefore considered to be externally
 visible; the
 sender\mbox{'}s \ensuremath{Choose} action, which changes only
 \ensuremath{y}, is considered to be an
 internal action. In other words, we consider \ensuremath{{\EE} y} :
 \ensuremath{Spec} to be the
 ``real'' specification.
\end{cpar}%
\end{lcom}%
\@x{ {\CONSTANT} Data}%
...
\@pvspace{8.0pt}%
\@x{ TypeOK \.{\defeq} \.{\land} x \.{\in} Data \.{\cup} \{ NonData \}}%
\@x{\@s{56.14} \.{\land} y\@s{0.10} \.{\in} {\SUBSET} Data}%
\@pvspace{8.0pt}%
\@x{ Init \.{\defeq} ( x \.{=} NonData ) \.{\land} ( y \.{=} \{ \} )}%
\@pvspace{8.0pt}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 00:40:17 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Wed} \ensuremath{Sep} 14
 03:25:35 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\end{lcom}%
\end{document}
