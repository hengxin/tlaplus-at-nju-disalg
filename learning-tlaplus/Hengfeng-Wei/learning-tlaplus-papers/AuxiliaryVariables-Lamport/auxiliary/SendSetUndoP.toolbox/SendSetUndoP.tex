...
This module is part of the example in Section 4.4 of the paper
 ``Auxiliary Variables in TLA+''. It defines the specification
 \ensuremath{SpecUP
} obtained, as explained in that paper, by adding a prophecy-array
 variable \ensuremath{p} to specification \ensuremath{SpecU} of module
 \ensuremath{SendSetUndo}. It then
 defines a refinement mapping under which \ensuremath{SpecUP} implements
 specification \ensuremath{Spec} of module \ensuremath{SendSet} to show that
 \ensuremath{SpecUP} implements
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The value of the variable \ensuremath{p} is always a function with domain
 \ensuremath{y}, which
 we call \ensuremath{Dom} to conform to the notation of Section 4.4 of
 ``Auxiliary
 Variables in TLA+''. The value of \ensuremath{p[d]} predicts whether the
 element \ensuremath{d
 } of \ensuremath{y} will be sent or simply undone--that is, removed from
 \ensuremath{y} by an
 \ensuremath{Undo(S)} action. These two predictions are represented by the
 string
 values \ensuremath{\@w{send}} and \ensuremath{\@w{undo}}.
\end{cpar}%
\end{lcom}%
\@x{ Pi\@s{10.73} \.{\defeq} \{\@w{send} ,\,\@w{undo} \}}%
\@x{ Dom \.{\defeq} y}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 As explained in the paper, the \ensuremath{SpecUP} is obtained by replacing
 each
 subaction A of \ensuremath{SpecU} with the subaction
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{T}{F}{7.5}{0}{}%
 \ensuremath{Ap \.{\defeq}} A \ensuremath{\.{\land} PredA(p) \.{\land}}
 (\ensuremath{p\.{'} \.{\in} NewPSetA(p)})
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{1}{F}{F}{0}{0}{}%
 For the reason explained below, we define \ensuremath{PredA} and
 \ensuremath{NewPSetA} before the
 variable \ensuremath{p} is declared, so we must define them to be operators
 that
 take \ensuremath{p} as an argument in the definition of \ensuremath{Ap}.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The definitions of \ensuremath{PredA} and \ensuremath{NewPSetA} are given
 below, for the four
 subactions A in the obvious disjunctive representation of
 \ensuremath{NextU}. (For
 example, \ensuremath{PredChoose} is \ensuremath{PredA} for A equal to
 \ensuremath{Choose}.)
\end{cpar}%
\end{lcom}%
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The following theorem must hold for \ensuremath{p} to be an auxiliary
 variable--that
 is, for \ensuremath{{\EE} p} : \ensuremath{SpecUP} to be equivalent to
 \ensuremath{SpecU}. It is equivalent to
 the conjunction of (4.11) of ``Auxiliary Variables in TLA+'' for the four
 subactions A. Note that we need each \ensuremath{PredA} to be an operator in
 order
 to state this condition. We make \ensuremath{NewPSetA} an operator as well
 so we
...
To check this theorem with TLA+, the module must be temporarily ended
 after the theorem so a model can be created having \ensuremath{SpecU} as its
 specification.
\end{cpar}%
\end{lcom}%
 \@x{ Condition \.{\defeq} {\Box} [ \.{\land} Choose \.{\implies} \E\, f
 \.{\in} [ Dom \.{\rightarrow} Pi ] \.{:} PredChoose ( f )}%
 \@x{\@s{73.09} \.{\land} Send\@s{9.42} \.{\implies} \E\, f \.{\in} [ Dom
 \.{\rightarrow} Pi ] \.{:} PredSend ( f )}%
 \@x{\@s{73.09} \.{\land} Rcv\@s{14.35} \.{\implies} \E\, f \.{\in} [ Dom
 \.{\rightarrow} Pi ] \.{:} PredRcv ( f )}%
 \@x{\@s{73.09} \.{\land} \A\, S \.{\in} ( {\SUBSET} y ) \.{\,\backslash\,} \{
 \{ \} \}\@s{4.1} \.{:}}%
 \@x{\@s{95.52} Undo ( S ) \.{\implies} \E\, f \.{\in} [ Dom \.{\rightarrow}
 Pi ] \.{:} PredUndo ( f ,\, S )}%
\@x{\@s{70.31} ]_{ vars}}%
\@pvspace{8.0pt}%
\@x{ {\THEOREM} SpecU \.{\implies} Condition}%
\@x{}\midbar\@xx{}%
\@x{ {\VARIABLE} p}%
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 Since \ensuremath{Dom} equals \ensuremath{y}, which initially equals the
 empty set, the initial
 value of \ensuremath{p} is the unique function with empty domain.
\end{cpar}%
\end{lcom}%
...
\@pvspace{8.0pt}%
 \@x{ NextUP \.{\defeq} ChooseP \.{\lor} SendP \.{\lor} RcvP \.{\lor} ( \E\, S
 \.{\in} ( {\SUBSET} y ) \.{\,\backslash\,} \{ \{ \} \} \.{:} UndoP ( S ) )}%
\@pvspace{8.0pt}%
\@x{ SpecUP\@s{1.08} \.{\defeq} InitUP \.{\land} {\Box} [ NextUP ]_{ varsP}}%
\@x{}\midbar\@xx{}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The \ensuremath{{\INSTANCE}} statement and theorem assert that
 \ensuremath{SpecUP} implements
 specification \ensuremath{Spec} of module \ensuremath{SendSet} under the
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 00:47:46 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created Sun \ensuremath{Sep} 25 05:58:07
 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\end{lcom}%
\end{document}
