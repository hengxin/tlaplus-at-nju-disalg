...
This module is part of the trivial example from Section 5 of the paper
 ...
 \ensuremath{HourMin} and \ensuremath{Stuttering}.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The module begins by defining a trivial specification \ensuremath{Spec} of a
 24-hour
 clock that begins with \ensuremath{h\.{=}0} and cycles through the integers
 mod 24.
\end{cpar}%
\end{lcom}%
\@x{ {\EXTENDS} Integers}%
\@x{ {\VARIABLE} h}%
\@pvspace{8.0pt}%
\@x{ TypeOK \.{\defeq} h \.{\in} 0 \.{\dotdot} 24}%
\@pvspace{8.0pt}%
\@x{ Init\@s{4.12} \.{\defeq} h \.{=} 0}%
\@pvspace{8.0pt}%
\@x{ Next \.{\defeq} h \.{'} \.{=} ( h \.{+} 1 ) \.{\%} 24}%
\@pvspace{8.0pt}%
\@x{ Spec\@s{1.46} \.{\defeq} Init \.{\land} {\Box} [ Next ]_{ h}}%
\@x{}\midbar\@xx{}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 We now add a stuttering variable \ensuremath{s} that adds 59 stuttering steps
 before
 each \ensuremath{Next} step, using the \ensuremath{PreStutter} operator of
 module \ensuremath{Stuttering}.
 The set Sigma of possible values \ensuremath{s.val} for the stuttering
 variable will
 be \ensuremath{1\.{\dotdot}59}, taking \ensuremath{\.{>}} as the
 ``less-than'' ordering on Sigma. Thus, the
 minimal value bot of Sigma is 59, the initial value \ensuremath{s.val} is
 set to is
 1, and the decrement operator maps a value \ensuremath{i} to
 \ensuremath{i\.{+}1}. The following
 assumption and theorem are the correctness conditions for adding
 stuttering steps to \ensuremath{Next}.
\end{cpar}%
\end{lcom}%
\@pvspace{8.0pt}%
 \@x{ {\THEOREM} Spec \.{\implies} {\Box} [ Next \.{\implies} ( 1 \.{\in} 1
 \.{\dotdot} 59 ) ]_{ h}}%
\@pvspace{8.0pt}%
\@x{ vars \.{\defeq} h}%
\@x{ {\VARIABLE} s}%
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
This is the constant condition that is required for \ensuremath{s} to be an
 auxiliary variable.
\end{cpar}%
\end{lcom}%
 \@x{ {\ASSUME} StutterConstantCondition ( 1 \.{\dotdot} 59 ,\, 59 ,\,
 {\LAMBDA} j \.{:} j \.{+} 1 )}%
\@pvspace{8.0pt}%
\@x{ TypeOKS \.{\defeq} \.{\land} TypeOK}%
 \@x{\@s{61.50} \.{\land} s \.{\in} \{ top \} \.{\cup} [ id \.{:} \{\@w{Next}
 \} ,\, ctxt \.{:} \{\@w{} \} ,\, val \.{:} 1 \.{\dotdot} 59 ]}%
\@pvspace{8.0pt}%
\@x{ InitS\@s{4.12} \.{\defeq} Init \.{\land} ( s \.{=} top )}%
\@pvspace{8.0pt}%
 \@x{ NextS \.{\defeq} PreStutter ( Next ,\, {\TRUE} ,\,\@w{Next} ,\,\@w{} ,\,
 59 ,\, 1 ,\, {\LAMBDA} j \.{:} j \.{+} 1 )}%
\@pvspace{8.0pt}%
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The following \ensuremath{{\INSTANCE}} statement and theorem show that
 \ensuremath{SpecS} implements
 Spec of module \ensuremath{HourMin} under a suitable refinement mapping,
 showing
 that \ensuremath{{\EE} s} : \ensuremath{SpecS} is equivalent to
 \ensuremath{Spec}.
\end{cpar}%
\end{lcom}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 \ensuremath{Oct} 20 05:46:14 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created Sun \ensuremath{Oct} 02 07:32:03
