...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 This is part of the \ensuremath{SendSeq} example, as explained in the
 comments in
 module \ensuremath{SendSeq}. The specification \ensuremath{SpecU} defined
 here is
 straightforward, except perhaps for the definition of
 \ensuremath{RemoveEltFrom}.
\end{cpar}%
\end{lcom}%
\@x{ {\EXTENDS} SendSeq}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{RemoveElt(i,\, seq)} is the sequence obtained from
 \ensuremath{seq} by removing element
 number \ensuremath{i} from it, assuming \ensuremath{seq} is a sequence and
 \ensuremath{1 \.{\leq} i \.{\leq} Len(seq)}.
 (The meaning of \ensuremath{RemoveElt(i,\, seq)} affects the specification
 only if
 those assumptions hold. This fact is implicitly checked by \ensuremath{TLC},
 which
 will report an error if checking the spec requires it to evaluate the
 expression when those assumptions don\mbox{'}t hold.) The definition is
 simple, since a sequence of length \ensuremath{n} is a function with domain
 \ensuremath{1\.{\dotdot}n}.
 However, it\mbox{'}s easy to make am ``off by one'' error in such a
 definition,
 so it\mbox{'}s a good idea to check it for a few values of \ensuremath{i}
 and \ensuremath{seq} using the
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 00:50:17 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Thu} \ensuremath{Sep} 15
 02:23:08 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\end{lcom}%
\end{document}
