...
This module adds a history variable \ensuremath{h} to the algorithm in module
 \ensuremath{AfekSimplified} and shows that the resulting specification
 \ensuremath{SpecH
 } implements specification \ensuremath{SafeSpec} of module
 \ensuremath{NewLinearSnapshot} under a
 suitable refinement mapping. This shows that specification \ensuremath{Spec}
 of
 module \ensuremath{AfekSimplified} implies \ensuremath{{\EE} mem},
 \ensuremath{rstate}, \ensuremath{wstate} : \ensuremath{SafeSpec}.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The history variable \ensuremath{h} is modified by the \ensuremath{BeginRd},
 \ensuremath{EndRd}, and \ensuremath{DoWr
 } actions exactly the way the corresponding actions of
 \ensuremath{NewLinearSnapshot
 } change \ensuremath{rstate}, so the refinement mapping can instantiate
 \ensuremath{rstate} with \ensuremath{h}.
 The instantiations of the other internal variables of
 \ensuremath{NewLinearSnapshot
} are straightforward.
\end{cpar}%
\end{lcom}%
...
\@pvspace{8.0pt}%
 \@x{ TypeOKH \.{\defeq} TypeOK \.{\land} ( h \.{\in} [ Readers
 \.{\rightarrow} Seq ( MemVals ) ] )}%
\@pvspace{8.0pt}%
 \@x{ InitH \.{\defeq} Init \.{\land} ( h \.{=} [ i \.{\in} Readers
 \.{\mapsto} {\langle} {\rangle} ] )}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 We define \ensuremath{memBar} to be the value of the variable
 \ensuremath{mem} of
 \ensuremath{NewLinearSnapshot} represented by \ensuremath{imem}. It is used
 both to instantiate
 the variable \ensuremath{mem} and in the definitions of the value of
 \ensuremath{h\.{'}} in some of
 the actions.
\end{cpar}%
\end{lcom}%
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 We instantiate \ensuremath{wstate} with the following expression
 \ensuremath{wstateBar}.
\end{cpar}%
\end{lcom}%
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 Here is the \ensuremath{{\INSTANCE}} statement and theorem asserting that
 \ensuremath{SpecH
 } implements \ensuremath{SafeSpec} of module \ensuremath{NewLinearSnapshot}
 under the refinement
 mapping. This theorem implies that the algorithm implements
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{T}{F}{7.5}{0}{}%
 \ensuremath{{\EE} mem}, \ensuremath{rstate}, \ensuremath{wstate} :
 \ensuremath{Spec
}%
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{1}{F}{F}{0}{0}{}%
 where \ensuremath{Spec} is the specification in
 \ensuremath{NewLinearSnapshot}.
\end{cpar}%
\end{lcom}%
\@x{ NLS \.{\defeq} {\INSTANCE} NewLinearSnapshot}%
 \@x{\@s{51.71} {\WITH} mem \.{\leftarrow} memBar ,\, rstate \.{\leftarrow} h
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 02:03:17 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Wed} \ensuremath{Oct} 05
 09:45:14 \ensuremath{PDT} 2016 by \ensuremath{lamport
}%
\end{cpar}%
\end{lcom}%
\end{document}
